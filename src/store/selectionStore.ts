...
import { VIDEO_LOSS_CONFIG } from '../constants/videoModels';
    ...
    immer((set, get) => ({
      ...
      addReferenceImage: async (src, blob?: Blob) => {
           ...
           set((state) => {
              ...
              if (isVideoMode) {
                   const config = VIDEO_LOSS_CONFIG[state.videoModel as keyof typeof VIDEO_LOSS_CONFIG];
                   if (config) max = config.max;
              }
