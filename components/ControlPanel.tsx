...
import { Wand2, Loader2, ImagePlus, Layers, X, Upload, Plus, Move, Sparkles, Minus, Maximize2, Banana, ChevronLeft, ChevronRight, Check, Clapperboard, Film, HelpCircle, LayoutGrid, MonitorPlay, Zap, Pin, PinOff, Eraser, Trash2 } from 'lucide-react';
...
const ControlPanel: React.FC<ControlPanelProps> = React.memo(({ onInitGenerations, onUpdateGeneration, onUpdateProgress, onOpenBatchModal }) => {
  ...
  */
  ...
  // 娴兼ê瀵查幓鎰仛鐠囧秴顦╅悶?(Modified to support Video type)
  ...
  const getImageTitleIcon = () => {
    ...
    if (imageLine === 'line2' || imageLine === 'line3') {
      return (
    ...
    return <Banana size={20} className="text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.5)] transform -rotate-12" />;
  };
  ...
  const videoTitleText =      <span className="bg-linear-to-r from-blue-300 via-purple-300 to-indigo-300 bg-clip-text text-transparent italic">
      AIGC Video
      </span>;
  ...
  return (
    ...
    >
      {autoCollapsed && !isMobile ? (
        ...
        <>
          {/* 婢舵挳鍎?- UNIFIED HEADER WITH TITLE */}
          <div 
        ...
        <div className={`p-4 ${isMobile ? 'pt-3' : ''} flex flex-col gap-4 overflow-y-auto overflow-x-hidden sleek-scroll-y ${isMobile ? 'max-h-[calc(86dvh-92px)] pb-[max(1rem,env(safe-area-inset-bottom))]' : 'max-h-[85vh]'}`}>
          ...
          <form onSubmit={handleFormSubmit} className="flex flex-col gap-3">
            ...
            <button
              ...
              className={`w-full ${isMobile ? 'py-3.5 rounded-xl text-base min-h-[50px]' : 'py-2.5 rounded-lg text-sm'} font-medium flex items-center justify-center gap-2 transition-all touch-manipulation active:scale-[0.98] ${isGenerating || (!prompt.trim() && toolMode !== ToolMode.INPAINT) ? 'bg-gray-700 text-gray-500 cursor-not-allowed' : 'bg-linear-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white shadow-lg'}`}
            >
              {isGenerating ? (
